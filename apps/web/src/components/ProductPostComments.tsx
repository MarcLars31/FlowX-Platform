...
export function ProductPostComments({ projectId, requirementId, productNumber, productName, disabled, onDirtyChange, onSavingChange, children }: {
  ...
  async function remove(comment: ProductPostComment, scope: "post" | "product") {
    ...
    try {
      ...
      window.requestAnimationFrame(() => document.getElementById(`comment-${scope}-${requirementId}`)?.focus({ preventScroll: true }));
  ...
  function section(scope: "post" | "product") {
    ...
    return <section aria-label={title} className="overflow-hidden rounded-lg border border-neutral-200 bg-white">
      <div className="p-4">
        ...
        {showFeedback && error && <p role="alert" className="mt-3 rounded-md bg-neutral-50 p-3 text-sm text-neutral-800">{error}</p>}
        {showFeedback && notice && <p role="status" className="mt-3 rounded-md bg-neutral-50 p-3 text-sm font-semibold text-neutral-800">{notice}</p>}
        {loading && <p role="status" className="mt-3 text-sm text-neutral-500">Hämtar kommentarer…</p>}
        {scopedComments.length > 0 && <ul aria-label={`Sparade ${scope === "post" ? "postkommentarer" : "produktkommentarer"}`} className="mt-4 space-y-3 border-t border-neutral-200 pt-4">
