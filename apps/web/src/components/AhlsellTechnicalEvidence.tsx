...
export function AhlsellTechnicalEvidence({ candidate }: { candidate: AhlsellPublicCandidate }) {
  ...
  return <details className="mt-2 rounded border border-neutral-200 bg-white text-xs text-neutral-800">
    <summary className="cursor-pointer px-2 py-2 font-semibold">Tekniskt produktunderlag · {keys.length} fält</summary>
    <p className="px-2 pb-2 text-neutral-600">Uppgifter för NRF {evidence.articleNumber}. Saknade uppgifter behöver kompletteras.</p>
    <dl className="divide-y divide-neutral-100">
      {keys.map(key => {
        const field = evidence.fields[key]!;
        return <div key={key} className={`px-2 py-2 ${field.status === "conflict" ? "bg-neutral-50" : ""}`}>
